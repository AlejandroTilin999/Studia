...
class ReporteController extends Controller
{
    ...
    {
        $workingPeriod = AcademicPeriod::where('status', AcademicPeriod::STATUS_ACTIVE)->first()
            ?? AcademicPeriod::where('status', AcademicPeriod::STATUS_PLANNING)->first();

        return Inertia::render('Admin/Reportes/Index', [
            // El generador inicia con el ciclo vigente: evita cargar alumnos y
            // grupos históricos que no pueden producir un reporte actual.
            'groups' => Inertia::defer(fn() => $workingPeriod
                ? AcademicGroup::query()
                    ->select('grupos.id', 'grupos.nombre')
                    ->join('inscripciones', 'inscripciones.grupo_id', '=', 'grupos.id')
                    ->where('inscripciones.ciclo_id', $workingPeriod->id)
            ...
            'students' => Inertia::defer(fn() => $workingPeriod
                ? Enrollment::select('id', 'usuario_id', 'grupo_id', 'codigo_alumno')
                    ->where('ciclo_id', $workingPeriod->id)
            ...
            'periods' => Inertia::defer(fn() => $workingPeriod
                ? collect([['id' => $workingPeriod->id, 'nombre' => $workingPeriod->nombre]])
                : collect()
            ),
    ...
    /** Descarga una lista de asistencia compatible con Microsoft Excel. */
    public function exportAttendanceCsv($groupId, $periodId)
    {
        $data = $this->makeAttendanceData($groupId, $periodId);
        $filename = 'asistencia-' . str($data['group']['nombre'])->slug() . '-' . $periodId . '.csv';

        return response()->streamDownload(function () use ($data) {
            $output = fopen('php://output', 'w');
            // UTF-8 BOM: Excel reconoce correctamente los acentos.
            fwrite($output, "\xEF\xBB\xBF");
            fputcsv($output, ['Lista de asistencia', $data['group']['nombre'], $data['period']['nombre']]);
            fputcsv($output, []);
            fputcsv($output, array_merge(['#', 'Matricula', 'Nombre'], range(1, 20)));

            foreach ($data['enrollments'] as $index => $enrollment) {
                fputcsv($output, array_merge([$index + 1, $enrollment['matricula'], $enrollment['nombre']], array_fill(0, 20, '')));
            }
            fclose($output);
