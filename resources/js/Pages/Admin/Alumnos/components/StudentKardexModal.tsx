...
export default function StudentKardexModal({
    ...
    const gpa = isNaN(parseFloat(rawGpa)) ? rawGpa : parseFloat(rawGpa);
    ...
    const kardexColumns: AppTableColumn<KardexRow>[] = [
        {
            header: "Ciclo / Periodo",
            accessor: (grade: KardexRow, idx: number) => {
                const isFirstOfPeriod = idx === 0 || displayGrades[idx - 1].period !== grade.period;
                return isFirstOfPeriod ? (
                    <div>
                        <span className="text-slate-800 font-extrabold block text-xs">{grade.period}</span>
                    </div>
                ) : null;
            },
            align: "left",
            className: "text-slate-500 font-semibold pl-4 text-left w-1/4"
        },
        {
            header: "Materia",
            accessor: (grade: KardexRow) => grade.subject,
            align: "left",
            className: "text-slate-755 font-bold pl-4 text-left w-2/4"
        },
        {
            header: "Calificación",
            accessor: (grade: KardexRow) => Number(grade.score).toFixed(1),
            align: "center",
            headerClassName: "text-center",
            className: "text-slate-700 font-extrabold text-center w-1/4"
        }
    ];
    ...
    return (
        ...
        >
            <div className="grid grid-cols-1 md:grid-cols-5 min-h-0 md:min-h-[480px] h-full text-left relative">
                ...
                <div className="col-span-1 md:col-span-2 bg-[#0266E0] p-6 text-white flex flex-col justify-between select-none relative rounded-t-[10px] md:rounded-l-[10px] md:rounded-tr-none">
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="py-2">
                                <span className="text-[10px] uppercase font-black text-blue-200 block mb-1">Promedio General</span>
                                <span className="text-4xl font-black text-white">{gpa}</span>
                            </div>
                        </div>
                    </div>
                ...
                <form onSubmit={handleDownloadSubmit} className="col-span-1 md:col-span-3 p-6 flex flex-col justify-between min-h-0 md:min-h-[480px] relative">
