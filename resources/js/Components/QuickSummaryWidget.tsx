        ...
        {metrics.map((item, index) => {
          ...
          return (
            <div
              key={index}
              className={cn(
                "border border-slate-100 shadow-none rounded-xl bg-white group transition-all duration-200 cursor-default p-6 flex flex-col gap-1.5 text-left",
            ...
            >
              ...
              <p className={cn(
                "text-[10px] md:text-[11px] font-black uppercase leading-none mt-1 transition-colors truncate",
                showSkeleton ? "text-slate-300" : "text-slate-550 group-hover:text-[#1e88e5]"
              )}>
                {item.label}
              </p>
              ...
              {showSkeleton ? (
                ...
                <p className="text-xl md:text-2xl font-black text-slate-800 mt-2 tracking-tight animate-in fade-in duration-500">{item.value}</p>
              )}
            </div>
          );
        })}
